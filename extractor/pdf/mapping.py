...
for d in json_collection:
    if d['type'] == "statement_of_income":
        ...
        json_copy = d #collect json if its balance type statement of income
        print("*" * 30)
        print(d)
        print("*" * 30)
...
def inject_db(json_data,latest_enum):
    ...
    if con is not None:
        ...
        for data in json_data["period"]:
            if data["asof"][-4:] ==  latest_enum[0] : #check year match
            ...
            for code in data["statement"]:
                ...
                if json_data["company"] == "paypal holdings inc":
                    if code["code"] == "AM_IS_E":
                        ...
                        SGA = 0
                        for i in code["items"]:
                            if re.search('Sales and marketing', i['desc']):
                                SGA += i['value']
                            if re.search('Product development', i['desc']):
                                SGA += i['value']
                            if re.search('General and administrative', i['desc']):
                                SGA += i['value']
                        AM_IS_EXP = SGA
...
inject_db(json_copy,latest_enum) #inject to database
